...
def run_validation(model: nn.Module, validation_ds: DataLoader, tokenizer_src: Tokenizer, tokenizer_tgt: Tokenizer, max_len: int, device: torch.device, print_msg: callable, global_step: int, num_examples: int = 2):
    ...
    print_msg(f"validation/cer: {float(cer):.6f} global_step: {global_step}")

    # Compute the word error rate
    metric = torchmetrics.WordErrorRate()
    wer = metric(predicted, expected)
    print_msg(f"validation/wer: {float(wer):.6f} global_step: {global_step}")

    # Compute the BLEU metric
    metric = torchmetrics.BLEUScore()
    bleu = metric(predicted, expected)
    print_msg(f"validation/BLEU: {float(bleu):.6f} global_step: {global_step}")
...
def train_model(config: ModelConfig):
    ...
    # No external experiment logger initialized
    ...
    if config.global_rank == 0:
        # No external experiment logger configured; metrics printed instead
        pass
    ...
    for epoch in range(initial_epoch, config.num_epochs):
        ...
        for batch in batch_iterator:
            ...
            if config.local_rank == 0:
                # Console log the loss for the rank 0 process
                batch_iterator.write(f"train/loss: {loss.item():.6f} global_step: {global_step}")
...
if __name__ == '__main__':
    ...
    # removed experiment logger CLI argument
